...
class OnnxWrapper {
 public:
  ...
  ///
  ////////////////////////////////////////////////////////////////////////////////////////////////
  template<typename T> Ort::Value vec_to_tensor(std::vector<T>& data, const std::vector<std::int64_t>& shape);
 ...
 private:
  ...
  std::vector<std::int64_t> output_shapes_;  // Shape of the outputs of the network
  ...
  int total_number_elements_;  // Total number of elements in the input of the network
...
template <typename T>
Ort::Value OnnxWrapper::vec_to_tensor(std::vector<T>& data, const std::vector<std::int64_t>& shape) {
...
void OnnxWrapper::initialize() {
  ...
  for (std::size_t i = 0; i < session_->GetInputCount(); i++) {
    ...
    input_shapes_ = session_->GetInputTypeInfo(i).GetTensorTypeAndShapeInfo().GetShape();
    std::cout << "\t" << input_names_.at(i) << " : " << print_shape(input_shapes_) << std::endl;
  }
  ...
  for (std::size_t i = 0; i < session_->GetOutputCount(); i++) {
    ...
    output_shapes_ = session_->GetOutputTypeInfo(i).GetTensorTypeAndShapeInfo().GetShape();
    std::cout << "\t" << output_names_.at(i) << " : " << print_shape(output_shapes_) << std::endl;
  }
  ...
  input_tensors_.emplace_back(vec_to_tensor<float>(input_tensor_values, input_shape));

  // double-check the dimensions of the input tensor
  assert(input_tensors_[0].IsTensor() && input_tensors_[0].GetTensorTypeAndShapeInfo().GetShape() == input_shape);
  std::cout << "\ninput_tensor shape: " << print_shape(input_tensors_[0].GetTensorTypeAndShapeInfo().GetShape()) << std::endl;
  ...
  try {
    auto output_tensors = session_->Run(Ort::RunOptions{nullptr}, input_names_char.data(), input_tensors_.data(),
                                      input_names_char.size(), output_names_char.data(), output_names_char.size());
    std::cout << "Done!" << std::endl;

    // double-check the dimensions of the output tensors
    // NOTE: the number of output tensors is equal to the number of output nodes specifed in the Run() call
    assert(output_tensors.size() == output_names_.size() && output_tensors[0].IsTensor());
...
VectorM OnnxWrapper::run(Eigen::VectorXf v) {
  ...
  // Double-check the dimensions of the input tensor
  assert(input_tensors_[0].IsTensor() && input_tensors_[0].GetTensorTypeAndShapeInfo().GetShape() == input_shapes_);
